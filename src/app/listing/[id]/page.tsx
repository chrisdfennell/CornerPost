...
export default async function ListingPage({
  ...
  // Log a view hit inside SQLite database
  try {
    await prisma.listingView.create({ data: { listingId: id } });
  } catch (err) {
    console.error("[analytics] Failed to log listing view:", err);
  }
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
      ...
      <nav className="flex flex-wrap items-center gap-1.5 text-sm text-slate-500 dark:text-slate-400">
        ...
        {cat && (
          <>
            <Link href={`/category/${cat.slug}`} className="hover:text-brand-600 dark:hover:text-brand-400">
              {cat.name}
            </Link>
            <span>/</span>
          </>
      ...
      <div className="mt-5 grid grid-cols-1 gap-8 lg:grid-cols-[1.6fr_1fr]">
        ...
        <div>
          ...
          <article className="mt-6 whitespace-pre-line text-[15px] leading-relaxed text-slate-700 dark:text-slate-350">
            {listing.description}
          </article>
        </div>
        ...
        <aside className="space-y-4">
              ...
              {place && (
                <div className="flex justify-between">
                  ...
                  <dd className="font-medium text-ink">
                    <Link href={`/api/place?slug=${place.slug}&next=/browse`} className="hover:text-brand-600 dark:hover:text-brand-400">
                      {place.label}
                    </Link>
                  </dd>
                </div>
          ...
          <ListingMap location={listing.location} placeLabel={place?.label ?? ""} />
