  ...
  if (!open) {
    return (
      <button
        ...
        className="mx-auto block text-xs font-medium text-slate-400 underline-offset-2 hover:text-slate-600 hover:underline"
      >
        🚩 Report this listing
      </button>
    );
  ...
  return (
    <form onSubmit={submit} className="space-y-3 rounded-2xl border border-slate-200 bg-white p-4">
      ...
      <textarea
        ...
        className="w-full resize-y rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-100"
      />
